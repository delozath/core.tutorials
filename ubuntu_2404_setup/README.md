...
## 1. 🛠️ Herramienta de postinstalación para Ubuntu 24.04 LTS

Postinstalación sobria para dejar una máquina con Ubuntu 24.04 LTS lista para trabajo técnico, desarrollo y cómputo científico. 🧭

La idea de este directorio es concentrar scripts reproducibles, comentados y fáciles de auditar para preparar una estación de trabajo sin depender de pasos manuales dispersos.

### 1.1. 📋 Resumen de lo que se instala

| Contenido |
| --- |
| Base del sistema y herramientas de administración. |
| Toolchain de desarrollo y compilación. |
| Python y dependencias comunes para trabajo científico. |
| Docker, editores, utilerías CLI y entorno académico. |
| LaTeX, multimedia, tipografías y productividad. |

### 1.2. 📌 Estado actual

| Script | Descripción |
| --- | --- |
| `apt_ubuntu24.04.sh` | instalación principal vía `apt`. |

### 1.3. 🧭 Criterios

- 🛡️ Priorizar repositorios oficiales o ampliamente mantenidos.
- ⚙️ Mantener el script legible, modular y seguro para reejecutar con criterio.
- 🧪 Separar lo esencial de lo opcional para no inflar la instalación base.
- 📦 Documentar qué se instala y por qué.
...
```bash
chmod +x ubuntu_2404_setup/apt_ubuntu24.04.sh
./ubuntu_2404_setup/apt_ubuntu24.04.sh
```

### 1.5. 📁 Convención de este directorio

- Los scripts deben validar entorno antes de modificar el sistema.
- El script incluye secciones comentadas de `snap` y `flatpak` para herramientas externas no disponibles en `apt`.

### 1.6. 🔭 Siguiente evolución

- Añadir scripts separados para `pip`/`uv`, `conda` o `mamba`.
- Incorporar postajustes de GNOME, fuentes, terminal y shells.
- Documentar perfiles por uso: docencia, ciencia de datos, desarrollo general, multimedia.

---

## 2. 🚀 Setup TensorFlow y PyTorch + CUDA en Ubuntu 24.04 LTS
...
#### 2.2.1. 💾 Reubicación del Almacenamiento de Docker (Data-Root)
...
### 2.3. 🗂️ Estructura de los Proyectos TensorFlow y PyTorch
...
### 2.4. ⚙️ Instalación de CUDA Toolkit + Docker NVIDIA Container Toolkit
...
### 2.5. 🧱 Imagenes base de Docker para TensorFlow y PyTorch
